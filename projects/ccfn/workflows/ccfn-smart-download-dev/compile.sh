#!/usr/bin/env bash
# compile.sh — compile the CCFN workflow, install dependencies, then patch WSL -> Windows paths
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTPUT_DIR="$SCRIPT_DIR/ecoscope-workflows-ccfn-smart-download-workflow"
